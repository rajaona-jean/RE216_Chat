...
int main(int argc, char** argv){
	...
	char* msg = malloc(512*sizeof(char));
	char* oth_pseudo2 =malloc(512*sizeof(char)); //pseudo de destination
	char * message_msg = malloc(512*sizeof(char));
					...
					if(i!=0){ // On parle avec le client
						...
						if(sl_check!=0){
							if(sl_check == 1){// On verifie /nick
								short_to_del=see_connected_user(liste,client_sock,server_sock,0);
								if(*short_to_del == 0){//Le client n'est pas connecté
									...
									to_del = get_nick(buffer,sl_check);
									strcpy(msg ,to_del);
							...
							if(sl_check == 2){//Who
								...
								do_write(client_sock,server_sock);
								short_to_del=see_connected_user(liste,client_sock,server_sock,1);
								free(short_to_del);
								memset(buffer,'\0',512);
								strcpy(buffer,"1");
							...
							if(sl_check==3){ //quit
								short_to_del = pseudo_known(liste,client_sock);
								if(*short_to_del==0){// pas de pseudo relatif au num de socket
									free(short_to_del);
									do_write(client_sock,server_sock);
									for(i=1;i<N;i++){
							...
							if(sl_check==5){//msgall
								...
								memset(buffer,'\0',512);
								sprintf(buffer," [%s] (public): ", sender);
								...
								if(ok_msgall ==1){
									memset(buffer,'\0',512);
									strcpy(buffer, "Message sent to all users");
									do_write(client_sock,server_sock);
									memset(buffer,'\0',512);
								}
								...
								if(ok_msgall==0){
									memset(buffer,'\0',512);
									strcpy(buffer, "No one else is connected");
									do_write(client_sock,server_sock);
									memset(buffer,'\0',512);
								}
							}
							...
							if(sl_check==6){ //msg
								...
								memset(message_msg,'\0',512);
								memset(oth_pseudo2,'\0',512);
								strcpy(message_msg,get_nick(buffer,sl_check)) ; 		//		chaine de caractere apres le /msg
								strcpy(sender,get_pseudo_from_sock(liste,client_sock));    // pseudo de celui qui envoie
								...
								for(k=0;k<length;k++){
									if(stop == 0){
										if(message_msg[k] != ' '){
											oth_pseudo2[k] = message_msg[k]; //pseudo du desinataire
											c++;
										}
								...
								sock_dest = -1;
								sock_dest =client_sock_from_pseudo(liste,oth_pseudo2);
								printf("msg %s\n", message_msg);
								fflush(stdout);
								printf("oth pseudo 2 %s\n", oth_pseudo2);
								fflush(stdout);
								printf("sock dest %d\n", sock_dest);
								fflush(stdout);


								if(sock_dest != -1 && sock_dest != client_sock){
									//envoi du message au destinataire
									memset(buffer,'\0',512);
									sprintf(buffer," [%s](private): ", sender);
									...
									strcat(buffer, oth_pseudo2);
									do_write(client_sock,server_sock);
									memset(buffer,'\0',512);
								}
								else if(sock_dest == client_sock){
									memset(buffer,'\0',512);
									strcpy(buffer, "[SERVER]: Sorry you can't send a message to yourself");
									do_write(client_sock,server_sock);
									memset(buffer,'\0',512);
								}
								else if(sock_dest == -1){
									memset(buffer,'\0',512);
									strcpy(buffer, "[SERVER]: Sorry this user does not exist ");
									do_write(client_sock,server_sock);
									memset(buffer,'\0',512);
								}
